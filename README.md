...
- [Watch or download the screen recording](demo/company-intel-screen-recording.mp4) — under three minutes, 1080p, synthetic narration and captions.
- [Earlier 2:51 explanatory walkthrough](demo/company-intel-demo.mp4), retained as supplementary material.
- [Demo guide, transcript, checks and regeneration instructions](demo/README.md).
- [Latest recorded three-company output](output/screen-demo-final/output.json), generated by the CLI shown in the screen recording.
- [Submission email draft](submission-email.md), including the explicit operations-question answer.

The primary video is a Playwright screen capture of a local browser workspace displaying the real source
files, executing the Python CLI, and opening the resulting JSON. Only idle waiting time is removed.
The earlier explanatory video uses rendered code views and excerpts from actual terminal output.
...
```powershell
.\.venv\Scripts\python.exe verify_outputs.py output/screen-demo-final
```
